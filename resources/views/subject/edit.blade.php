			...
			<form action="{{ route('subjects.update', $subject->id) }}" method="post">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control @error('kode_matkul') is-invalid @enderror" name="kode_matkul"
						id="kode_matkul" placeholder="Kode Matkul" autocomplete="off"
						value="{{ old('kode_matkul') ?? $subject->kode_matkul }}">
					@error('kode_matkul')
				...
				<div class="form-group">
					...
					<input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
						placeholder="name" autocomplete="off" value="{{ old('name') ?? $subject->name }}">
					@error('name')
						<span class="text-danger">{{ $message }}</span>
					@enderror
				</div>
				...
				<div class="form-group">
					...
					<input type="number" min="1" class="form-control @error('sks') is-invalid @enderror" name="sks"
						id="sks" placeholder="sks" autocomplete="off" value="{{ old('sks') ?? $subject->sks }}">
					@error('sks')
						<span class="text-danger">{{ $message }}</span>
					@enderror
				</div>
				...
				<div class="form-group">
					...
					<select name="study_program_id" class="form-control @error('study_program_id') is-invalid @enderror">
						@foreach (\App\StudyProgram::get() as $program)
							<option value="{{ $program->id }}" {{ $program->id == $subject->study_program_id ? 'selected' : '' }}>
								{{ $program->name }}</option>
						@endforeach
					</select>
